...
import { Code, Database, Layout, PenTool, Terminal, Sparkles, Rocket } from 'lucide-react';
...
export const Skills: React.FC = () => {
  return (
    <Section id="skills" title="Technical Proficiency" subtitle="Tools and technologies I work with" className="bg-slate-50">
        ...
        {SKILLS.map((category, idx) => {
          ...
          return (
            <motion.div
              key={idx}
              whileHover={{ y: -5 }}
              className="bg-slate-50 rounded-xl p-6 border border-slate-100 hover:border-primary-200 transition-all duration-300 shadow-sm hover:shadow-md w-full md:w-[calc(50%-12px)] lg:w-[30%]"
            >
                ...
                {category.skills.map((skill, sIdx) => (
                  <div key={sIdx} className="relative flex flex-col items-center group">
                    ...
                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-slate-800 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 pointer-events-none shadow-lg z-20 whitespace-nowrap">
                      {skill.funTooltip || skill.name}
                      <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-slate-800 rotate-45"></div>
                    </div>

                    <div className="h-10 w-10 mb-2 transition-transform duration-300 group-hover:scale-110">
                      <img
                        src={skill.iconKey.startsWith('http') ? skill.iconKey : `https://cdn.jsdelivr.net/gh/devicons/devicon/icons/${skill.iconKey}.svg`}
                        alt={skill.name}
